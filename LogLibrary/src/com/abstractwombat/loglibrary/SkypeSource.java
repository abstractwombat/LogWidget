...
public class SkypeSource implements ALogSource, NotificationHistory.Specialization, NotificationSource {
	private static final String TAG = "SkypeSource";
    ...
    private final String INTENT_ACTION_OPENSKYPE = "open_skype";
	...
	public void config(Context context, LogSourceConfig config){
		...
		this.config = (SkypeSourceConfig)config;
        ...
        this.db.addPackageFilter(SKYPE_PACKAGE);
        ...
        cache = new ImageCache("SkypeLogSource");
    }
	...
	public RemoteViews getViewAt(int position){
        ...
        SkypeNotificationInfo info = extractSkypeInfo(data);
        ...
        rowIntent.putExtra(context.getPackageName() + ".action", INTENT_ACTION_OPENSKYPE);
        ...
        if (this.config.showImage){
            ...
            if (image == null) {
                ...
                if (config.showEmblem) {
                    Bitmap emblem = BitmapFactory.decodeResource(context.getResources(), R.drawable.skype_emblem);
        ...
        if (config.bubbleResourceName.isEmpty()) {
            ...
            if (foundBubbleResource) {
                ...
                bubbleResId = SkypeSourceConfig.DEFAULT_BUBBLE_RESOURCE;
            }
            ...
            LogSourceFactory.newSource(context, SkypeSource.class, config);
        }else{
    ...
    private class SkypeNotificationInfo {
        long time;
        String name;
        String message;
        String phoneNumber;
    ...
    private SkypeNotificationInfo extractSkypeInfo(NotificationHistory.NotificationData data){
        SkypeNotificationInfo info = new SkypeNotificationInfo();
        info.time = data.time;
    ...
    public boolean confirmAdd(NotificationHistory.NotificationData toAdd) {
        ...
        if (this.config.lookupKeyFilter != null && this.config.lookupKeyFilter.length > 0) {
            SkypeNotificationInfo info = extractSkypeInfo(toAdd);
    ...
    public boolean areCollapsible(NotificationHistory.NotificationData a, NotificationHistory
            .NotificationData b) {
        SkypeNotificationInfo aInfo = extractSkypeInfo(a);
        SkypeNotificationInfo bInfo = extractSkypeInfo(b);
	...
	public void receiveIntent(Context context, Intent intent){
		...
		if (action.equals(INTENT_ACTION_OPENSKYPE)){
            ...
            if (name != null && !name.isEmpty()) {
                Intent skypeIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("skype:" + name +
                        "?chat"));
                skypeIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(skypeIntent);
            } else {
                // Can't find the contact, just launch Skype app
                PackageManager manager = context.getPackageManager();
                Intent launchIntent = manager.getLaunchIntentForPackage(SKYPE_PACKAGE);
