...
public class FacebookMessengerSource implements ALogSource, NotificationHistory.Specialization, NotificationSource {
	private static final String TAG = "FacebookMessengerSource";
    public static final String FACEBOOKMESSENGER_PACKAGE = "com.facebook.orca";
    ...
    private final String INTENT_ACTION_OPENFACEBOOKMESSENGER = "open_facebookmessenger";
    ...
    FacebookMessengerSourceConfig config;

    public class FacebookMessengerData{
        long date;
        String key;
        String name;
        Bitmap image;
	...
	FacebookMessengerSource(){
	}

	@Override
	public void config(Context context, LogSourceConfig config){
		this.context = context;
		this.config = (FacebookMessengerSourceConfig)config;
        ...
        this.db.addPackageFilter(FACEBOOKMESSENGER_PACKAGE);
        ...
        cache = new ImageCache("FacebookMessengerLogSource");
    }
	...
	public RemoteViews getViewAt(int position){
        ...
        FacebookMessengerNotificationInfo info = extractFacebookMessengerInfo(data);
        ...
        rowIntent.putExtra(context.getPackageName() + ".action", INTENT_ACTION_OPENFACEBOOKMESSENGER);
        ...
        if (this.config.showImage){
            ...
            if (image == null) {
                ...
                if (config.showEmblem) {
                    Bitmap emblem = BitmapFactory.decodeResource(context.getResources(), R.drawable.facebook_messenger_emblem);
        ...
        rv.setTextColor(R.id.date, Color.argb(Math.round(.54f * 255.f), textR, textG, textB));
        ...
        if (config.bubbleResourceName.isEmpty()) {
            ...
            if (foundBubbleResource) {
                ...
                bubbleResId = FacebookMessengerSourceConfig.DEFAULT_BUBBLE_RESOURCE;
            }
            ...
            LogSourceFactory.newSource(context, FacebookMessengerSource.class, config);
        }else{
    ...
    private class FacebookMessengerNotificationInfo {
        long time;
        String name;
        String message;
        String phoneNumber;
    ...
    private FacebookMessengerNotificationInfo extractFacebookMessengerInfo(NotificationHistory.NotificationData data){
        FacebookMessengerNotificationInfo info = new FacebookMessengerNotificationInfo();
        info.time = data.time;
        ...
        if (info.name != null && info.name.contains(",")){
            // If the name is comma delimited names, then sort them
    ...
    public String getPackage() {
        return FACEBOOKMESSENGER_PACKAGE;
    }
    ...
    public boolean confirmAdd(NotificationHistory.NotificationData toAdd) {
        ...
        if (this.config.lookupKeyFilter != null && this.config.lookupKeyFilter.length > 0) {
            FacebookMessengerNotificationInfo info = extractFacebookMessengerInfo(toAdd);
    ...
    public boolean areCollapsible(NotificationHistory.NotificationData a, NotificationHistory
            .NotificationData b) {
        FacebookMessengerNotificationInfo aInfo = extractFacebookMessengerInfo(a);
        FacebookMessengerNotificationInfo bInfo = extractFacebookMessengerInfo(b);
	...
	public void receiveIntent(Context context, Intent intent){
		...
		if (action.equals(INTENT_ACTION_OPENFACEBOOKMESSENGER)){
            ...
            // Open the Messenger app to the appropriate conversation
            //  - This requires a facebook user id, which can be obtained using their Graph API.
//            Uri uri = Uri.parse("fb-messenger://user/");
//            uri = ContentUris.withAppendedId(uri, 1013532967);  // This is krista's ID taken from her profile URL.
//            Intent openMess = new Intent(Intent.ACTION_VIEW, uri);
//            openMess.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//            openMess.setPackage(FACEBOOKMESSENGER_PACKAGE);
//            context.startActivity(openMess);

            // Launch FacebookMessenger app
            PackageManager manager = context.getPackageManager();
            Intent launchIntent = manager.getLaunchIntentForPackage(FACEBOOKMESSENGER_PACKAGE);
