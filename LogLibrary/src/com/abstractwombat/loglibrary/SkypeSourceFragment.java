...
public class SkypeSourceFragment extends ALogSourcePreferenceFragment {
	private static final String TAG = "SkypeSourceFragment";

    @Override
    protected int getPreferences() {
        return R.xml.skypesource_preferences;
    }
    ...
    public void onResume() {
        ...
        SkypeSource source = (SkypeSource)LogSourceFactory.get(this.context, this.mLogSourceId);
        ...
        for (int p=0; p<screen.getPreferenceCount(); p++){
            ...
            if (key == null || key.equals("skypesource_enable")){
    ...
    protected void storeToFactory(){
		...
		EditTextPreference editCount = (EditTextPreference)findPreference("skypesource_count");
		CheckBoxPreference chkImage = (CheckBoxPreference)findPreference("skypesource_show_contact_image");
		CheckBoxPreference chkName = (CheckBoxPreference)findPreference("skypesource_show_name");
		CheckBoxPreference chkDate = (CheckBoxPreference)findPreference("skypesource_long_date");
        EditTextPreference editMaxLines = (EditTextPreference)findPreference("skypesource_maxlines");
        ResourcePreference bubbleStyle = (ResourcePreference)findPreference("skypesource_bubblestyle");
        ColorPreference rowColor = (ColorPreference)findPreference("skypesource_rowcolor");
        ColorPreference textColor = (ColorPreference)findPreference("skypesource_textcolor");
        ColorPreference bubbleColor = (ColorPreference)findPreference("skypesource_bubblecolor");
        CheckBoxPreference chkEmblem = (CheckBoxPreference)findPreference("skypesource_show_emblem");
        EditTextPreference textSize = (EditTextPreference)findPreference("skypesource_textsize");
		...
		SkypeSourceConfig config = new SkypeSourceConfig(initConfig);
        Resources res = getResources();
        try {
            config.count = Integer.parseInt(editCount.getText());
        }catch (NumberFormatException e){
            config.count = res.getInteger(R.integer.skype_source_default_count);
        }
		...
		LogSourceFactory.newSource(this.context, SkypeSource.class, config);
    ...
    protected void loadFromFactory(){
        ...
        EditTextPreference editMaxLines = (EditTextPreference)findPreference("skypesource_maxlines");
        ResourcePreference bubbleStyle = (ResourcePreference)findPreference("skypesource_bubblestyle");
        ColorPreference editRowColor = (ColorPreference)findPreference("skypesource_rowcolor");
        ColorPreference editTextColor = (ColorPreference)findPreference("skypesource_textcolor");
        ColorPreference bubbleColor = (ColorPreference)findPreference("skypesource_bubblecolor");
        CheckBoxPreference chkEmblem = (CheckBoxPreference)findPreference("skypesource_show_emblem");
        EditTextPreference textSize = (EditTextPreference)findPreference("skypesource_textsize");
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
            PreferenceCategory cat = (PreferenceCategory) findPreference("skypesource_category_appearance");
		...
		if (source == null){
			...
			Resources res = getResources();
			editCount.setText(Integer.toString(res.getInteger(R.integer.skype_source_default_count)));
			chkImage.setChecked(res.getBoolean(R.bool.skype_source_default_showimage));
			chkName.setChecked(res.getBoolean(R.bool.skype_source_default_showname));
			chkDate.setChecked(res.getBoolean(R.bool.skype_source_default_longdate));
            editMaxLines.setText(Integer.toString(res.getInteger(R.integer
                    .skype_source_default_maxlines)));
            bubbleStyle.setDefaultValue(SkypeSourceConfig.DEFAULT_BUBBLE_RESOURCE);
            chkEmblem.setChecked(res.getBoolean(R.bool.skype_source_default_showemblem));
            if (textSize != null) textSize.setText(Integer.toString(res.getInteger(R.integer.skype_source_default_textsize)));
            bubbleColor.setDefaultValue(SkypeSourceConfig.DEFAULT_BUBBLE_COLOR);
            editRowColor.setDefaultValue(Integer.toString(res.getColor(R.color
                    .skype_source_default_rowcolor)));
            editTextColor.setDefaultValue(Integer.toString(res.getColor(R.color.skype_source_default_textcolor)));
            Log.d(TAG, "Loaded defaults");
		}else{
			// Set from the config
			SkypeSourceConfig config = (SkypeSourceConfig)source.config();
            if (config.count < 0){
