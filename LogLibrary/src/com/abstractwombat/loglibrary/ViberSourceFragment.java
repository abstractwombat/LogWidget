...
public class ViberSourceFragment extends ALogSourcePreferenceFragment {
	private static final String TAG = "ViberSourceFragment";

    @Override
    protected int getPreferences() {
        return R.xml.vibersource_preferences;
    }
    ...
    public void onResume() {
        ...
        ViberSource source = (ViberSource)LogSourceFactory.get(this.context, this.mLogSourceId);
        ...
        for (int p=0; p<screen.getPreferenceCount(); p++){
            ...
            if (key == null || key.equals("vibersource_enable")){
    ...
    protected void storeToFactory(){
		...
		EditTextPreference editCount = (EditTextPreference)findPreference("vibersource_count");
		CheckBoxPreference chkImage = (CheckBoxPreference)findPreference("vibersource_show_contact_image");
		CheckBoxPreference chkName = (CheckBoxPreference)findPreference("vibersource_show_name");
		CheckBoxPreference chkDate = (CheckBoxPreference)findPreference("vibersource_long_date");
        EditTextPreference editMaxLines = (EditTextPreference)findPreference("vibersource_maxlines");
        ResourcePreference bubbleStyle = (ResourcePreference)findPreference("vibersource_bubblestyle");
        ColorPreference rowColor = (ColorPreference)findPreference("vibersource_rowcolor");
        ColorPreference textColor = (ColorPreference)findPreference("vibersource_textcolor");
        ColorPreference bubbleColor = (ColorPreference)findPreference("vibersource_bubblecolor");
        CheckBoxPreference chkEmblem = (CheckBoxPreference)findPreference("vibersource_show_emblem");
        EditTextPreference textSize = (EditTextPreference)findPreference("vibersource_textsize");
		...
		ViberSourceConfig config = new ViberSourceConfig(initConfig);
        Resources res = getResources();
        try {
            config.count = Integer.parseInt(editCount.getText());
        }catch (NumberFormatException e){
            config.count = res.getInteger(R.integer.viber_source_default_count);
        }
		...
		LogSourceFactory.newSource(this.context, ViberSource.class, config);
    ...
    protected void loadFromFactory(){
        ...
        EditTextPreference editMaxLines = (EditTextPreference)findPreference("vibersource_maxlines");
        ResourcePreference bubbleStyle = (ResourcePreference)findPreference("vibersource_bubblestyle");
        ColorPreference editRowColor = (ColorPreference)findPreference("vibersource_rowcolor");
        ColorPreference editTextColor = (ColorPreference)findPreference("vibersource_textcolor");
        ColorPreference bubbleColor = (ColorPreference)findPreference("vibersource_bubblecolor");
        CheckBoxPreference chkEmblem = (CheckBoxPreference)findPreference("vibersource_show_emblem");
        EditTextPreference textSize = (EditTextPreference)findPreference("vibersource_textsize");
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
            PreferenceCategory cat = (PreferenceCategory) findPreference("vibersource_category_appearance");
		...
		if (source == null){
			...
			Resources res = getResources();
			editCount.setText(Integer.toString(res.getInteger(R.integer.viber_source_default_count)));
			chkImage.setChecked(res.getBoolean(R.bool.viber_source_default_showimage));
			chkName.setChecked(res.getBoolean(R.bool.viber_source_default_showname));
			chkDate.setChecked(res.getBoolean(R.bool.viber_source_default_longdate));
            editMaxLines.setText(Integer.toString(res.getInteger(R.integer
                    .viber_source_default_maxlines)));
            bubbleStyle.setDefaultValue(ViberSourceConfig.DEFAULT_BUBBLE_RESOURCE);
            chkEmblem.setChecked(res.getBoolean(R.bool.viber_source_default_showemblem));
            if (textSize != null) textSize.setText(Integer.toString(res.getInteger(R.integer.viber_source_default_textsize)));
            bubbleColor.setDefaultValue(ViberSourceConfig.DEFAULT_BUBBLE_COLOR);
            editRowColor.setDefaultValue(Integer.toString(res.getColor(R.color
                    .viber_source_default_rowcolor)));
            editTextColor.setDefaultValue(Integer.toString(res.getColor(R.color.viber_source_default_textcolor)));
            Log.d(TAG, "Loaded defaults");
		}else{
			// Set from the config
			ViberSourceConfig config = (ViberSourceConfig)source.config();
            if (config.count < 0){
