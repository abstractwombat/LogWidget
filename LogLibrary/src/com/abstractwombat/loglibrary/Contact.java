...
//    };
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!MessagingUtils.isEmailAddress(number)) {
    ...
    public synchronized void setNumber(String number) {
        if (!MessagingUtils.isEmailAddress(number)) {
            mNumber = PhoneNumberUtils.formatNumber(number);
        } else {
            mNumber = number;
        }
    ...
    public synchronized boolean isEmail() {
        return MessagingUtils.isEmailAddress(mNumber);
    }
    ...
    private static class ContactsCache {
        ...
        private Contact get(String number, boolean isMe, boolean canBlock) {
            Log.d("Contact", "get(" + number + ", " + isMe + ", " + canBlock + ")");
            ...
            synchronized (contact) {
                ...
                if (contact.mIsStale && !contact.mQueryPending) {
                    ...
                    Log.d("Contact", "async update for " + contact.toString() + " canBlock: "
                            + canBlock +
        ...
        private void updateContact(final Contact c) {
            ...
            synchronized (c) {
                if (contactChanged(c, entry)) {
                    Log.d("Contact", "updateContact: contact changed for " + entry.mName);
        ...
        private Contact getContactInfo(Contact c) {
            ...
            } else if (MessagingUtils.isEmailAddress(c.mNumber) || isAlphaNumber(c.mNumber)) {
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            Log.d("Contact", "queryContactInfoByNumber: number=" + number);


            String normalizedNumber = ContactUtilities.removeNonNumericCharacters(number);
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                String numberE164 = PhoneNumberUtils.formatNumber(number);
                String selection;
                String[] args;
        ...
        private void fillPhoneTypeContact(final Contact contact, final Cursor cursor) {
            synchronized (contact) {
                ...
                Log.d("Contact", "fillPhoneTypeContact: name=" + contact.mName + ", number="
        ...
        private void fillSelfContact(final Contact contact, final Cursor cursor) {
            synchronized (contact) {
                ...
                if (TextUtils.isEmpty(contact.mName)) {
                    contact.mName = "No name";
                }
                Log.d("Contact", "fillSelfContact: name=" + contact.mName + ", number="
                        + contact.mNumber);
            }
            byte[] data = loadAvatarData(contact);
        ...
        private byte[] loadAvatarData(Contact entry) {
            ...
            Log.d("Contact", "loadAvatarData: name=" + entry.mName + ", number=" + entry.mNumber);
                ...
                try {
                    while (cursor.moveToNext()) {
                        ...
                        synchronized (entry) {
                            ...
                            if (!TextUtils.isEmpty(name)) {
                                entry.mName = name;
                                Log.d("Contact", "getContactInfoForEmailAddress: name=" +
                                            entry.mName +
        ...
        private Contact internalGet(String numberOrEmail, boolean isMe) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = isMe || MessagingUtils.isEmailAddress(numberOrEmail);
        ...
        private void remove(Contact contact) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = contact.isMe() ||
                                    MessagingUtils.isEmailAddress(number);
                final String key = isNotRegularPhoneNumber ?
