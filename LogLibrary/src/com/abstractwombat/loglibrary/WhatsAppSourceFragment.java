...
public class WhatsAppSourceFragment extends ALogSourcePreferenceFragment {
	private static final String TAG = "WhatsAppSourceFragment";
    ...
    public void onResume() {
        ...
        WhatsAppSource source = (WhatsAppSource)LogSourceFactory.get(this.context, this.mLogSourceId);
    ...
    protected void storeToFactory(){
		...
		EditTextPreference editCount = (EditTextPreference)findPreference("whatsappsource_count");
		CheckBoxPreference chkImage = (CheckBoxPreference)findPreference("whatsappsource_show_contact_image");
		CheckBoxPreference chkName = (CheckBoxPreference)findPreference("whatsappsource_show_name");
		CheckBoxPreference chkDate = (CheckBoxPreference)findPreference("whatsappsource_long_date");
        EditTextPreference editMaxLines = (EditTextPreference)findPreference("whatsappsource_maxlines");
        ResourcePreference bubbleStyle = (ResourcePreference)findPreference("whatsappsource_bubblestyle");
        ColorPreference rowColor = (ColorPreference)findPreference("whatsappsource_rowcolor");
        ColorPreference textColor = (ColorPreference)findPreference("whatsappsource_textcolor");
        ColorPreference bubbleColor = (ColorPreference)findPreference("whatsappsource_bubblecolor");
        CheckBoxPreference chkEmblem = (CheckBoxPreference)findPreference("whatsappsource_show_emblem");
        EditTextPreference textSize = (EditTextPreference)findPreference("whatsappsource_textsize");
		...
		WhatsAppSourceConfig config = new WhatsAppSourceConfig(initConfig);
        Resources res = getResources();
        try {
            config.count = Integer.parseInt(editCount.getText());
        }catch (NumberFormatException e){
            config.count = res.getInteger(R.integer.whatsapp_source_default_count);
        }
		...
		LogSourceFactory.newSource(this.context, WhatsAppSource.class, config);
    ...
    protected void loadFromFactory(){
        ...
        EditTextPreference editMaxLines = (EditTextPreference)findPreference("whatsappsource_maxlines");
        ResourcePreference bubbleStyle = (ResourcePreference)findPreference("whatsappsource_bubblestyle");
        ColorPreference editRowColor = (ColorPreference)findPreference("whatsappsource_rowcolor");
        ColorPreference editTextColor = (ColorPreference)findPreference("whatsappsource_textcolor");
        ColorPreference bubbleColor = (ColorPreference)findPreference("whatsappsource_bubblecolor");
        CheckBoxPreference chkEmblem = (CheckBoxPreference)findPreference("whatsappsource_show_emblem");
        EditTextPreference textSize = (EditTextPreference)findPreference("whatsappsource_textsize");
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
            PreferenceCategory cat = (PreferenceCategory) findPreference("whatsappsource_category_appearance");
		...
		if (source == null){
			...
			Resources res = getResources();
			editCount.setText(Integer.toString(res.getInteger(R.integer.whatsapp_source_default_count)));
			chkImage.setChecked(res.getBoolean(R.bool.whatsapp_source_default_showimage));
			chkName.setChecked(res.getBoolean(R.bool.whatsapp_source_default_showname));
			chkDate.setChecked(res.getBoolean(R.bool.whatsapp_source_default_longdate));
            editMaxLines.setText(Integer.toString(res.getInteger(R.integer
                    .whatsapp_source_default_maxlines)));
            editRowColor.setDefaultValue(Integer.toString(res.getColor(R.color
                    .whatsapp_source_default_rowcolor)));
            bubbleStyle.setDefaultValue(WhatsAppSourceConfig.DEFAULT_BUBBLE_RESOURCE);
            chkEmblem.setChecked(res.getBoolean(R.bool.whatsapp_source_default_showemblem));
            if (textSize != null) textSize.setText(Integer.toString(res.getInteger(R.integer.whatsapp_source_default_textsize)));
            bubbleColor.setDefaultValue(WhatsAppSourceConfig.DEFAULT_BUBBLE_COLOR);
            editTextColor.setDefaultValue(Integer.toString(res.getColor(R.color.whatsapp_source_default_textcolor)));
            Log.d(TAG, "Loaded defaults");
		}else{
			// Set from the config
			WhatsAppSourceConfig config = (WhatsAppSourceConfig)source.config();
            if (config.count < 0){
