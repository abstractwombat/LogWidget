...
public class ViberSource implements ALogSource, NotificationHistory.Specialization, NotificationSource {
	private static final String TAG = "ViberSource";
    ...
    private final String INTENT_ACTION_OPENVIBER = "open_viber";
	...
	public void config(Context context, LogSourceConfig config){
		...
		this.config = (ViberSourceConfig)config;
        ...
        this.db.addPackageFilter(VIBER_PACKAGE);
        ...
        cache = new ImageCache("ViberLogSource");
    }
	...
	public RemoteViews getViewAt(int position){
        ...
        ViberNotificationInfo info = extractViberInfo(data);
        ...
        rowIntent.putExtra(context.getPackageName() + ".action", INTENT_ACTION_OPENVIBER);
        ...
        if (this.config.showImage){
            ...
            if (image == null) {
                ...
                if (config.showEmblem) {
                    Bitmap emblem = BitmapFactory.decodeResource(context.getResources(), R.drawable.viber_emblem);
        ...
        if (config.bubbleResourceName.isEmpty()) {
            ...
            if (foundBubbleResource) {
                ...
                bubbleResId = ViberSourceConfig.DEFAULT_BUBBLE_RESOURCE;
            }
            ...
            LogSourceFactory.newSource(context, ViberSource.class, config);
        }else{
    ...
    private class ViberNotificationInfo {
        long time;
        String name;
        String message;
        String phoneNumber;
    ...
    private ViberNotificationInfo extractViberInfo(NotificationHistory.NotificationData data){
        ViberNotificationInfo info = new ViberNotificationInfo();
        info.time = data.time;
    ...
    public boolean confirmAdd(NotificationHistory.NotificationData toAdd) {
        ...
        if (this.config.lookupKeyFilter != null && this.config.lookupKeyFilter.length > 0) {
            ViberNotificationInfo info = extractViberInfo(toAdd);
    ...
    public boolean areCollapsible(NotificationHistory.NotificationData a, NotificationHistory
            .NotificationData b) {
        ViberNotificationInfo aInfo = extractViberInfo(a);
        ViberNotificationInfo bInfo = extractViberInfo(b);
	...
	public void receiveIntent(Context context, Intent intent){
		...
		if (action.equals(INTENT_ACTION_OPENVIBER)){
            ...
            if (lookupKey != null) {
                ...
                Cursor c = context.getContentResolver().query(ContactsContract.Data.CONTENT_URI,
                        ...
                        ContactsContract.Data.MIMETYPE + " = 'vnd.android.cursor.item/vnd.com.viber.voip.viber_number_message'",
                        null, null);
                ...
                if (c != null && c.moveToFirst()) {
                    do {
                        String viberNumber = c.getString(1);
                        viberNumber = ContactUtilities.formatPhoneNumberForStorage(viberNumber);
                        if (phoneNumbers.contains(viberNumber) /*&& mimeType.equals
                            (preferredMimeType)*/) {
                            contactId = c.getString(0);
                            Log.d(TAG, "Found the contact's Viber number (" + viberNumber + ") " +
                                    "and " +
                                    "contact id (" + contactId + ")");
                            foundNumber = viberNumber;
                            break;
                        }
                    } while (c.moveToNext());
                }
                ...
                if (contactId != null) {
                    // Show this contact in Viber
                    viberIntent = new Intent(Intent.ACTION_VIEW,
                            Uri.parse("content://com.android.contacts/data/" + contactId));
                    viberIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    viberIntent.setPackage(VIBER_PACKAGE);
                }
            }

            if (viberIntent == null) {
                // Can't find the contact, just launch Viber app
                PackageManager manager = context.getPackageManager();
                viberIntent = manager.getLaunchIntentForPackage(VIBER_PACKAGE);
                if (viberIntent != null) {
                    viberIntent.addCategory(Intent.CATEGORY_LAUNCHER);
                    viberIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                }
            }
            if (viberIntent != null) {
