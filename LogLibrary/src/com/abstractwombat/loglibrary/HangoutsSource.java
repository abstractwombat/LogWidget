...
public class HangoutsSource implements ALogSource, NotificationHistory.Specialization, NotificationSource {
	private static final String TAG = "HangoutSource";
    ...
    private final String INTENT_ACTION_OPENHANGOUTS = "open_hangouts";
	...
	public void config(Context context, LogSourceConfig config){
		...
		this.config = (HangoutsSourceConfig)config;
        ...
        this.db.addPackageFilter(HANGOUTS_PACKAGE);
        ...
        cache = new ImageCache("HangoutsLogSource");
    }
	...
	public RemoteViews getViewAt(int position){
        ...
        HangoutsNotificationInfo info = extractHangoutsInfo(data);

        // Construct the views
        RemoteViews rv = null;
//        if (info.isSMS) {
//            rv = new RemoteViews(this.context.getPackageName(), R.layout.sms_row_material);
//        }else{
...
//        }else {
            rowIntent.putExtra(context.getPackageName() + ".action", INTENT_ACTION_OPENHANGOUTS);
//        }
        ...
        if (this.config.showImage){
            ...
            if (image == null) {
                ...
                if (config.showEmblem) {
                    Bitmap emblem = BitmapFactory.decodeResource(context.getResources(), R.drawable.hangouts_emblem);
        ...
        if (config.bubbleResourceName.isEmpty()) {
            ...
            if (foundBubbleResource) {
                ...
                bubbleResId = HangoutsSourceConfig.DEFAULT_BUBBLE_RESOURCE;
            }
            ...
            LogSourceFactory.newSource(context, HangoutsSource.class, config);
        }else{
    ...
    private class HangoutsNotificationInfo {
        long time;
        String name;
        String message;
        String phoneNumber;
    ...
    private HangoutsNotificationInfo extractHangoutsInfo(NotificationHistory.NotificationData data){
        HangoutsNotificationInfo info = new HangoutsNotificationInfo();
        info.time = data.time;
        info.isSMS = false;
    ...
    private boolean isSmsMessage(NotificationHistory.NotificationData data){
        if (data.people == null) return false;
        if (data.people.length == 0) return false;
        if (data.people[0].length() == 0) return false;
        if (data.people[0].startsWith("tel:")){
            return true;
        } else {
            return false;
        }
    ...
    public boolean confirmAdd(NotificationHistory.NotificationData toAdd) {
        ...
        // Exclude if it's an SMS and we aren't showing those
//        if (!config.showSMS && isSmsMessage(toAdd)){
//            Log.d(TAG, "Not including sms");
//            return false;
//        }

        // Check if this matches the filter
        if (this.config.lookupKeyFilter != null && this.config.lookupKeyFilter.length > 0) {
            HangoutsNotificationInfo info = extractHangoutsInfo(toAdd);
    ...
    public boolean areCollapsible(NotificationHistory.NotificationData a, NotificationHistory
            .NotificationData b) {
//        if (isSmsMessage(a) != isSmsMessage(b)) return false;
        HangoutsNotificationInfo aInfo = extractHangoutsInfo(a);
        HangoutsNotificationInfo bInfo = extractHangoutsInfo(b);
	...
	public void receiveIntent(Context context, Intent intent){
		...
		if (action.equals(INTENT_ACTION_OPENHANGOUTS)){
            ...
            Integer[] rawIds = null;
            if (lookupUri == null) {
                rawIds = ContactUtilities.getRawIds(context, name);
            }else{
                rawIds = ContactUtilities.getRawIds(context, Uri.parse(lookupUri));
            }

            String where = ContactsContract.Data.MIMETYPE + " = 'vnd.android.cursor.item/vnd.googleplus.profile.comm' AND " + ContactsContract.Data.DATA4 + " = '10'";
            Integer[] hangoutIds = ContactUtilities.getDataIds(context, rawIds, where);
            if (hangoutIds.length > 0){
                // Show this contact in Hangouts
                Intent testH = new Intent(Intent.ACTION_VIEW);
                testH.setFlags(Intent.FLAG_ACTIVITY_FORWARD_RESULT | Intent.FLAG_ACTIVITY_PREVIOUS_IS_TOP | Intent.FLAG_RECEIVER_FOREGROUND);
                Uri uri = Uri.parse("content://com.android.contacts/data/" + hangoutIds[0].toString()); // Krista:1297
                testH.setDataAndType(uri, "vnd.android.cursor.item/vnd.googleplus.profile.comm");
                context.startActivity(testH);
            }else{
                // Launch Hangouts app
                PackageManager manager = context.getPackageManager();
                Intent launchIntent = manager.getLaunchIntentForPackage(HANGOUTS_PACKAGE);
        ...
        if (action.equals(INTENT_ACTION_SENDSMS)){
            ...
            sendIt(context, Intent.ACTION_SENDTO, "smsto:" + number);
            return;
        }
